...
namespace Gecko.Loggly.Net.Controllers
{
    [Authorize]
    public class WidgetsController : ApiController
    {
        ...
        public HttpResponseMessage Text([FromUri]string format = "JSON", [FromUri]string type = "4")
        {
            try
            {
                ...
                var retVal = new GeckoItems();
                var items = new List<DataItem>() { 
                    new DataItem(){ Text = "None", Type = DataItemType.None },
                    new DataItem(){ Text = "Info", Type = DataItemType.Info },
                    new DataItem(){ Text = "Alert", Type = DataItemType.Alert }
                };
                retVal.DataItems = items.ToArray();
